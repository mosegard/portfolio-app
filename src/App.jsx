...
function App() {
    ...
    // Use Persistent State for config so manual overrides are saved
    const [config, setConfig] = usePersistentState('portfolio_config', {
        askAccount: '',
        currencies: {},
        hidden: []
    });
    ...
    // --- FIX: Auto-detect currencies from transaction history ---
    const detectedCurrencies = useMemo(() => {
        const map = {};
        txs.forEach(t => {
            if (t.account && t.currency && !map[t.account]) {
                map[t.account] = t.currency;
            }
        });
        return map;
    }, [txs]);
    ...
    return (
        <div className="flex h-screen w-full bg-white relative font-sans text-gray-800">
                ...
                <div className="absolute inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl shadow-2xl p-6 w-96 space-y-4">
                        ...
                        <div className="flex items-center gap-2 pt-2">
                            ...
                            <label htmlFor="anonymity-blur-toggle" className="text-sm text-gray-700 cursor-pointer select-none">Slør app ved tab af fokus (anonymitet)</label>
                        </div>
                        ...
                        <button onClick={() => { setShowSettings(false); setShowCacheInspector(true); }} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-purple-700 bg-purple-50 hover:bg-purple-100 rounded-lg text-sm font-medium transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-2">
                    ...
                    <div>
                        ...
                        <div className="mt-1 space-y-0.5">
                            {accounts.filter(acc => !config.hidden.includes(acc)).map(acc => {
                                // FIX: Use detected currency if config is empty
                                const currency = config.currencies[acc] || detectedCurrencies[acc] || 'DKK';
                                return (
                                    <button key={acc} onClick={() => { setView('editor'); setFilterAccount(acc); }} className={`sub-nav-item ${view === 'editor' && filterAccount === acc ? 'active' : ''} flex justify-between items-center pr-2`}>
                                        <span className="truncate" title={acc}>{acc}</span>
                                        <span className="text-[10px] font-mono text-gray-400 bg-gray-100 px-1 rounded ml-2">{currency}</span>
                                    </button>
                                );
                            })}
                        </div>
            ...
            {/* Mobile Nav & Main Content structure preserved... */}
            {mobileNavOpen && (
                <div className="fixed inset-0 z-50 md:hidden">
                    ...
                    <div className="absolute top-0 left-0 w-64 h-full bg-white shadow-xl border-r border-gray-200 flex flex-col z-10">
                        ...
                        <div className="flex-1 overflow-y-auto p-3 space-y-2">
                             {/* Mobile nav items... same logic as desktop for currency */}
                             <button type="button" onClick={() => { setView('dashboard'); setMobileNavOpen(false); }} className={`nav-item w-full text-left ${view === 'dashboard' ? 'active' : ''}`}><i className="ph ph-chart-line-up text-lg"></i> Oversigt</button>
                             <button type="button" onClick={() => { setView('holdings'); setMobileNavOpen(false); }} className={`nav-item w-full text-left ${view === 'holdings' ? 'active' : ''}`}><i className="ph ph-briefcase text-lg"></i> Beholdninger</button>
                             <div>
                                <button type="button" onClick={() => { setView('editor'); setFilterAccount('All'); setMobileNavOpen(false); }} className={`nav-item w-full text-left ${view === 'editor' && filterAccount === 'All' ? 'active' : ''}`}><i className="ph ph-list-dashes text-lg"></i> Transaktioner</button>
                                <div className="mt-1 space-y-0.5">
                                    {accounts.filter(acc => !config.hidden.includes(acc)).map(acc => {
                                        const currency = config.currencies[acc] || detectedCurrencies[acc] || 'DKK';
                                        return (
                                            <button key={acc} onClick={() => { setView('editor'); setFilterAccount(acc); setMobileNavOpen(false); }} className={`sub-nav-item ${view === 'editor' && filterAccount === acc ? 'active' : ''} flex justify-between items-center pr-2`}>
                                                <span className="truncate" title={acc}>{acc}</span>
                                                <span className="text-[10px] font-mono text-gray-400 bg-gray-100 px-1 rounded ml-2">{currency}</span>
                                            </button>
                                        );
                             ...
                             {/* Rest of mobile nav... */}
                             <div>
                                <button type="button" onClick={() => { setView('tax'); setMobileNavOpen(false); }} className={`nav-item w-full text-left ${view === 'tax' ? 'active' : ''}`}><i className="ph ph-bank text-lg"></i> Skatterapport</button>
                                <div className="mt-1 space-y-0.5">
                                    {years.map(y => (
                                        <button type="button" key={y} onClick={() => { setView('tax'); setTaxYear(y); setMobileNavOpen(false); }} className={`sub-nav-item ${view === 'tax' && taxYear === y ? 'active' : ''}`}>{y}</button>
                                    ))}
                                </div>
                             </div>
                             <button type="button" onClick={() => { setView('split'); setMobileNavOpen(false); }} className={`nav-item w-full text-left ${view === 'split' ? 'active' : ''}`}><i className="ph ph-arrows-split text-lg"></i> Split-værktøj</button>
                        </div>
                        <div className="p-4 border-t border-gray-200 flex flex-col gap-2">
                            <button onClick={() => { setShowSettings(true); setMobileNavOpen(false); }} className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 text-gray-700"><i className="ph ph-gear-six"></i> Indstillinger</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden bg-white">
                <div className="h-14 border-b border-gray-100 flex items-center justify-between px-4 md:px-8 shrink-0 bg-white relative">
                    <button onClick={() => setMobileNavOpen(true)} className="md:hidden p-2 -ml-2 text-gray-700 hover:text-gray-900" aria-label="Open menu"><i className="ph ph-list text-2xl"></i></button>
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => fetchMarketData(true)} className="p-2 w-10 h-10 rounded-full bg-white border border-gray-200 hover:bg-blue-50 text-blue-600 text-lg shadow-sm flex items-center justify-center" title={lastUpdate ? `Sidst opdateret: ${lastUpdate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : "Opdater priser"}>
                ...
                {schemaErrors.length > 0 && (
                    <div className="bg-red-50 border-b border-red-200 p-4 max-h-40 overflow-y-auto">
                        ...
                        <ul className="list-disc list-inside text-sm text-red-700 space-y-1 font-mono">{schemaErrors.map((e, i) => <li key={i}>{e}</li>)}</ul>
                    </div>
                )}
                ...
                {calc.warnings && calc.warnings.length > 0 && (
                    <div className="bg-orange-50 border-b border-orange-200 p-4 max-h-40 overflow-y-auto animate-in slide-in-from-top-2">
                        ...
                        <div className="text-sm text-orange-800 mb-2">Følgende mangler forhindrer korrekt skatteberegning:</div>
                        <ul className="list-disc list-inside text-xs text-orange-800 space-y-1 font-mono bg-white/50 p-2 rounded border border-orange-100">{calc.warnings.map((w, i) => <li key={i}>{w}</li>)}</ul>
                    </div>
                )}
